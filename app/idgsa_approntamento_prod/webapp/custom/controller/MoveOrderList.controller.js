...
controller.AbstractController.extend("controller.MoveOrderList", {
    /**
     * @memberOf controller.MoveOrderList
     */
    onInit: function () {
    ...
    handleRouteMatched: function (evt) {
        if (!this._checkRoute(evt, "moveOrderNotYetWorked")) return;
        ...
        this.orderListModel.setProperty("/results", []);
        var fSuccess = function (result) {
            
            this.unfilteredMoveOrderList = result;
        ...
        var fError = function () {
            this.getView().setBusy(false);
            var that = this;
            sap.m.MessageBox.alert(this._getLocaleText("orderListErr"), {
        ...
        if (this.userLogged) {
            ...
            if (this.userLogged[0].agentCode) {
                req.agentCode = this.userLogged[0].agentCode;
            } else {
                req.agentCode = "";
            }
            if (!!currentCustomer) {
            ...
            //**
            model.MoveOrderModel.getMoveOrders(req).then(fSuccess, fError);
        } else {
    ...
    onOrderPress: function (evt) {
        ...
        this.router.navTo("orderDetail", {
                'orderId': selectedContext.orderId
            });
    },
    onFilterMoveOrderList: function () {
        this.orderListModel.setProperty("/results", this.unfilteredMoveOrderList);
    ...
    onFilterPress: function () {
        if (this._checkCustomerSession()) {
            this.filterModel = model.filters.Filter.getModel(this.orderListModel.getData().results, "ordersInSession");
        } else {
    ...
    onFilterDialogOK: function (evt) {
        if (this._checkCustomerSession()) {
            var filterItems = model.filters.Filter.getSelectedItems("ordersInSession");
        } else {
    ...
    onResetFilterPress: function () {
        ...
        this.uiModel.setProperty("/rangeVisible", false);
        if (this._checkCustomerSession()) {
            model.filters.Filter.resetFilter("ordersInSession");
        } else {
    ...
    onLinkToPdfPress: function (evt) {
        var orderId = evt.getSource().getBindingContext("orderListModel").getObject().orderId;
        var elementId = "#" + evt.getSource().getId();
        utils.Busy.show(this._getLocaleText("disablePopupBlocker"));
        var f = function () {
            model.MoveOrderModel.printOdV(orderId, false)
    ...
    loadDelivery: function (id, type) {
        ...
        fSuccess = _.bind(fSuccess, this);
        fError = _.bind(fError, this);
        model.MoveOrderModel.getDelivery(id, type).then(fSuccess, fError);
    ...
    onRemoveODataFilter: function (evt) {
        ...
        this.getView().setBusy(true);
        model.MoveOrderModel.getMoveOrders(reqData) // forceReload
            .then(_.bind(function (res) {
                // console.log(res);
                this.getView().setBusy(false);
                //this.setButtonVisible(res);
                this.orderListModel.setProperty("/results", res);
    ...
    onSearchOrderByVbeln: function (evt) {
        var orderId = evt.getSource().getValue();
        var req = this._getOrgData();
        if (req) {
            req.BukrsVf = req.Bukrs;
            delete req.Bukrs;
        ...
        fError = _.bind(fError, this);
        this.getView().setBusy(true);
        model.MoveOrderModel.getOrderByVbeln(req).then(fSuccess, fError);
    },
